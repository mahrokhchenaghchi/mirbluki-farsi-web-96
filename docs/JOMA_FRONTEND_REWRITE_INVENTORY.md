...
**به‌روزرسانی v4 (بعد از بازبینی نهایی):** صفحهٔ «جزئیات دوره» (`period.php`) هم کامل بازچینی شد
(سرصفحهٔ طرح تازه · نوار مسیر دوره · چهار کارت عدد · برنامهٔ قفل‌شدهٔ همان دوره · کارت «دورهٔ نو با همین
فعالیت‌ها» وقتی مجاز است · لینک خروجی داده) و «کنسول مدیر» و «حقوق داده» در سایدبار دسکتاپِ مدیر و در
گروه «بیشتر» منو قرار گرفتند. با این، **همهٔ صفحه‌های اپ طرح تازه دارند و هیچ صفحهٔ قدیمی نمانده است.**
آزمون دسترسی: کاربر عادی به کنسول «دسترسی مجاز نیست» می‌گیرد و لینکش را در منو نمی‌بیند.

**بستهٔ v5 (این دور):**
- `health.php` — صفحهٔ «بررسی سلامت نصب» با ۱۶ بررسی و متن راه‌حل فارسی (نسخهٔ PHP · افزونه‌های
  mbstring/json/session · نوشتنی بودن data و sessions و hammasir · ۱۳ فایل کلیدی · حفاظت .htaccess ·
  حالت ذخیره‌سازی · اتصال دیتابیس اگر MySQL باشد · کتابخانهٔ ۱۰۷ فعالیتی · ساعت/منطقهٔ زمانی ·
  بودن `_SEED-DEMO.php` و `install.php`). هیچ‌چیز را تغییر نمی‌دهد.
- آزمون سازگاری PHP: همان بسته روی **7.4، 8.0 و 8.3** اجرا شد — همهٔ صفحه‌ها ۲۰۰، صفر خطا،
  آزمون‌ها ۳۴+۲۹ سبز، خروجی‌های حقوق داده و ثبت لیوان هم درست. (PHP 8.4 تست نشده.)
- `03-نقشه-سایت-اصلی.txt` — سند رفتن به دامنه: یک سؤال باز از مالک (حالت فایل یا MySQL)، فهرست دقیق
  «چه آپلود شود و چه نشود» (config و data هرگز)، ترتیب کار با پشتیبان‌گیری، جدول ریسک و کاهش،
  چک‌لیست بعد از رفتن. **هیچ اجرایی روی سایت اصلی انجام نشده.**
- راهنمای‌های مرحله‌ای قدیمی به `test/docs/` منتقل شدند تا کنار راهنمای اصلی گیج‌کننده نباشند.

**بستهٔ v6 (این دور) — شکاف باقی‌ماندهٔ فاز ۳ پر شد:**
`includes/v2_calendar.php` — کامپوننت **«مسیر این ماه»** طبق `SPEC/11 §۷`: کاشی‌های سه‌بعدی پاستلی
با عدد روز (نعنا = کامل · طلایی = ناقص · آبی‌خاکستری = بی‌ثبت · هلویی = امروز · خط‌چین = آینده)،
راهنمای رنگی برچسب‌دار زیر تقویم (رنگ تنها حامل معنا نیست)، نوار خلاصه با لمس هر روز
(«کارها ۳ از ۵ · آب ۵ لیوان · حال ثبت شد») و لمس دوباره = رفتن به ثبت همان روز، پیام روشن برای
روزهای آینده («ثبت برای روزهای آینده ممکن نیست»)، چهار شمارنده (کامل · ناقص · بی‌ثبت · روزهای ماه)
و آمار واقعی (روزهای دارای ثبت · پرثبت‌ترین روز). **یک کامپوننت، دو جا:** خانهٔ من و تب
«مسیر این ماه» در کارهای امروز (`index.php?p=today&view=month`).

آزمون: کاربر با داده → ۳۲ کاشی و شمارش «۶ کامل · ۸ ناقص · ۱۳ بی‌ثبت» · کاربر تازه → ۳۲ کاشی با
۵ روز آیندهٔ خط‌چین و پیام ماه خالی · سه نقش × ۲۰ صفحه (شامل تب تقویم) → صفر خطای PHP ·
آزمون‌های خودکار ۳۴ + ۲۹ سبز.
