...
# پیوست ۶ — بستهٔ نهایی v4 (چهار فاز کامل + همهٔ اصلاح‌ها)

بستهٔ تحویلی: `joma/JOMA-TEST-PACK-v4.zip` — یک فایل، یک اکسترکت، ۱۲۹ فایل.

**چهار فاز:** ① اسکلت و زبان بصری (`joma-v2.css` + sprite جغد + پوسته) · ② ورود/ثبت‌نام/بازیابی رمز
(ویزارد ثبت‌نام ۵ گام، ویزارد حال ۶ گام، بازیابی با کد یک‌بارمصرف) · ③ قلب محصول (خانه · امروز ·
حال · جوجه · تقویم/گزارش‌های مرتبط) · ④ بقیهٔ صفحه‌ها و نقش‌ها (گزارش‌ها ۱۰ تب با نمودار ·
دفترچه · کتابخانه · آموزش · هم‌مسیر · تنظیمات · **حقوق داده** · **کنسول مدیر**).

**صفحه‌های تازه در فاز ۴:** `pages/rights.php` (خروجی واقعی CSV/JSON/گزارش خوانا + درخواست حذف با مهلت
و لغو؛ پاک‌سازی خودکار ۳۰ روزه صریحاً «نمایشی · منتظر بک‌اند») و `pages/admin_console.php`
(وضعیت نصب، کاربران، درخواست‌های حذف، پرثبت‌ترین فعالیت‌ها — بدون هیچ دادهٔ خصوصی؛ مدیر نمی‌تواند حساب حذف کند).

**اصلاح‌های این دور (بازخورد مالک):** صدای آب پیش‌فرض خاموش و نامش «صدای آب» · خالی‌کردن بی‌صدا ·
ثبت نهایی بدون پرش صفحه (دکمه غیرفعال می‌شود، چیدمان ثابت) · تمرین تنفس ۴-۷-۸ با صدا و انیمیشن و
پیش‌فرض خاموش · ویزاردهای ثبت‌نام (۵ گام) و حال (۵+۱ گام) · نمودار برای همهٔ تب‌های گزارش
(۱۰ تب: خلاصه ۱ · موفقیت ۵ · فعالیت‌ها ۱ · وزن ۱ · تقویم ۱+نقشهٔ حرارتی · روند ۱ · خلق ۱ ·
مقایسه ۲ با انتخاب دورهٔ دوم · جزئیات ۱ · تحلیلی ۱+R2) با پیام روشن در نبود داده.

**مستندات داخل بسته:** `00-راهنما-نصب.txt` · `01-چه-چیزی-در-این-بسته-هست.txt` ·
`02-گزارش-تست.txt` · `اختیاری-1-joma_settings.sql` (ADD-ONLY، فقط اگر روزی با MySQL اجرا شد).

**آزمون پذیرش روی خودِ فایل ZIP (اکسترکت تازه):** seed بدون هشدار · B6/B7 = ۳۴ سبز · B8 = ۲۹ سبز ·
سه نقش × ۲۳ صفحه، صفر خطا · کنسول مدیر سالم · هر ده تب گزارش بدون خطا · مخاطب بدون ورود هم
راهنما و صفحه‌های عمومی را می‌بیند.
