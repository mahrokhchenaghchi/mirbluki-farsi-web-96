...
?>
<?php joma_v2_pagehead('دوره‌های من', 'دوره‌های من', 'هر دوره تاریخچهٔ مستقل خودش را نگه می‌دارد؛ قفل برنامه هم همین‌جا معنا پیدا می‌کند.'); ?>
<form class="card grid grid-3" method="post">
  <div style="grid-column:1/-1"><b style="font-size:13px">ساخت یا انتخاب دوره</b><p class="tiny">دوره = یک ماه شمسی. برنامه، هدف‌ها و ثبت‌های همان ماه داخل آن می‌مانند.</p></div>
  <?php echo csrf_field(); ?>
...
foreach ($list as $p) {
    ...
    echo '<a href="'.e(joma_url('index.php?p=period&period='.$p['period_key'])).'" style="display:flex;gap:10px;align-items:center">'
       . '<span class="icon-btn">' . joma_v2_icon('i-cal') . '</span>'
       . '<span><h3 style="margin:0">'.e(jalali_period_label($p['period_key'])).'</h3><p class="tiny" dir="ltr">'.$p['start_date'].' — '.$p['end_date'].'</p></span></a>';
