...
function joma_v2_chick_svg($state = 'calm', $size = 150) {
    ...
    if ($state === 'happy') {
        ...
        $svg .= '<ellipse cx="30" cy="66" rx="6" ry="4" fill="' . $p['blush'] . '" opacity=".5"/>';
        $svg .= '<ellipse cx="90" cy="66" rx="6" ry="4" fill="' . $p['blush'] . '" opacity=".5"/>';
    } elseif ($state === 'sleep') {
        $svg .= '<path d="M38 54h12M70 54h12" stroke="' . $p['pupil'] . '" stroke-width="2.6" stroke-linecap="round"/>';
        $svg .= '<text x="92" y="30" font-size="15" fill="' . $p['ring'] . '" font-family="sans-serif">z</text>';
        $svg .= '<text x="102" y="20" font-size="11" fill="' . $p['ring'] . '" font-family="sans-serif">z</text>';
