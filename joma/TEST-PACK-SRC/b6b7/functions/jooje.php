...
function jooje_state($user_id) {
    ...
    $water_today = 0.0;
    $water_total = 0.0;
    $water_draft_today = false;
    foreach ($events as $e) {
        $is_today = ($e['performance_date'] === $today);
        if ($is_today) $seed_today++;
        if (isset($e['activity_code']) && $e['activity_code'] === 'ACT002') {
            // (B6/WTR-10) پیش‌نویس آب، سنجهٔ جوجه را پر نمی‌کند؛ فقط ثبت قطعی می‌شمارد.
            $final = function_exists('joma_water_is_final') ? joma_water_is_final($user_id, $e['performance_date']) : true;
            if (!$final) {
                if ($is_today) $water_draft_today = true;
                continue;
            }
    ...
    $water_metric = array(
        ...
        'draft_today' => $water_draft_today,   // (B6) پیش‌نویس امروز: شمرده نشده است
