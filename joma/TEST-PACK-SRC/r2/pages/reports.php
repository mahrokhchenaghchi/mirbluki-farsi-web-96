...
// JOMA-ANALYTICS-R1-BEGIN — تب «گزارش تحلیلی» فقط با لود موفق (fail-closed)
// (tabfix) ثبت تب باید «قبل از» نرمال‌سازی $tab باشد؛ در نسخه‌ی قبلی بعد از آن
// ثبت می‌شد، بنابراین tab=analytics همیشه به overview بازنشانی می‌شد و کلیکِ تب
// ظاهراً هیچ نمی‌کرد (شاخه‌ی رندر هرگز اجرا نمی‌شد).
...
if ($tab === 'overview') {
    ...
    if ($__analytics_r1_ok) {
        try {
            ...
            echo '<p class="lede">امکان تهیه گزارش در حال حاضر وجود ندارد.</p>';
        }
    } else {
        echo '<p class="lede">امکان تهیه گزارش در حال حاضر وجود ندارد.</p>';
    }
