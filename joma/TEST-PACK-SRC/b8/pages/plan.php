...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'add') {
        ...
        $wp = ensure_period($u['id'], $key);
        $plan = $wp['plan'];
    } elseif ($act === 'copy_next') {
        // (B8) کپی برنامهٔ همین دوره به ماه بعد — از اسنپ‌شات همین دوره، بدون هیچ رخداد/حالی.
        if (!function_exists('joma_copy_plan_to_next')) {
            $msg = 'این امکان در این نسخه فعال نیست.';
        } else {
            $res = joma_copy_plan_to_next($u['id'], $plan);
            if (!empty($res['ok'])) {
                set_current_period_key($res['target_key']);
                flash_set('ok', fa_num($res['copied']) . ' فعالیت به ' . jalali_period_label($res['target_key']) . ' منتقل شد. حالا هدف‌ها را برای ' . jalali_period_label($res['target_key']) . ' تنظیم کن — تا شروع اجرا، همه‌چیز قابل تغییر است.');
                joma_redirect('index.php?p=plan');
            } else {
                $msg = $res['error'];
            }
        }
...
?>
<?php joma_v2_pagehead('برنامهٔ من', 'برنامهٔ ' . jalali_period_label($plan['period_key']), 'هدف و اهمیت را برای همین ماه تعیین کن. بعد از نهایی‌سازی، تصویر برنامه قفل می‌شود تا تاریخچه سالم بماند.', status_badge($plan['status'])); ?>
<div class="card">
  <div class="k">مسیر برنامه</div>
  <div class="row" style="gap:6px;margin-top:6px">
    <?php
    $steps = array('DRAFT' => 'پیش‌نویس', 'PLANNING' => 'آمادهٔ اجرا', 'RUNNING' => 'در حال اجرا', 'ARCHIVED' => 'بایگانی');
    $order = array_keys($steps);
    $curIdx = array_search($plan['status'], $order, true);
    if ($curIdx === false) $curIdx = 0;
    foreach ($order as $i => $st) {
        $cls = ($i < $curIdx) ? 'g' : (($i === $curIdx) ? 's' : 'n');
        echo '<span class="chip ' . $cls . '">' . e($steps[$st]) . '</span>';
        if ($i < 3) echo '<span class="tiny">←</span>';
    }
    ?>
  </div>
  <p class="tiny" style="margin-top:6px">پیش‌نویس → آمادهٔ اجرا → در حال اجرا → بایگانی. تا وقتی «در حال اجرا» نشده، می‌شود برنامه را عوض کرد.</p>
</div>
<?php if ($msg) echo '<p class="toast bad">'.e($msg).'</p>'; ?>
<?php
/* (B8) کارت «دورهٔ نو با همین فعالیت‌ها» — فقط با رعایت پنج شرط سند. */
$__cs = function_exists('joma_copy_plan_status') ? joma_copy_plan_status($u['id'], $plan) : null;
if ($__cs) {
    $__show = in_array($__cs['reason'], array('', 'TARGET_LOCKED', 'TARGET_EXISTS'), true) || $__cs['can_copy'];
    if ($__show) { ?>
<section class="card" style="background:linear-gradient(150deg,#FFFFFF,var(--brand-softer))">
  <?php if (!empty($__cs['can_copy'])) { ?>
    <div class="row" style="justify-content:space-between">
      <div class="k">دورهٔ نو با همین فعالیت‌ها</div>
      <span class="chip g"><?php echo e($__cs['target_label']); ?></span>
    </div>
    <p class="lede" style="margin-top:6px">
      <?php echo fa_num($__cs['source_count']); ?> فعالیت و هدف‌هایشان از
      <?php echo e(jalali_period_label($plan['period_key'])); ?> به <?php echo e($__cs['target_label']); ?> منتقل می‌شود.
      ثبت‌ها و گزارش‌های <?php echo e(jalali_period_label($plan['period_key'])); ?> دست‌نخورده می‌ماند.
    </p>
    <p class="tiny">هدف‌ها را در <?php echo e($__cs['target_label']); ?> می‌توانی عوض کنی — تا وقتی اجرای <?php echo e($__cs['target_label']); ?> را شروع نکنی.</p>
    <form method="post" style="margin-top:8px">
      <?php echo csrf_field(); ?>
      <input type="hidden" name="action" value="copy_next">
      <div class="btn-row">
        <button class="btn" type="submit">بساز</button>
        <span class="tiny" style="align-self:center">فعلاً نه — همین‌جا بمان</span>
      </div>
    </form>
  <?php } else { ?>
    <div class="k">دورهٔ نو با همین فعالیت‌ها</div>
    <p class="lede" style="margin-top:6px"><?php echo e($__cs['message']); ?></p>
    <?php if ($__cs['reason'] === 'TARGET_EXISTS') { ?>
      <div class="btn-row" style="margin-top:8px">
        <a class="btn sec" href="<?php echo e(joma_url('index.php?p=period&period=' . $__cs['target_key'])); ?>">رفتن به <?php echo e($__cs['target_label']); ?></a>
      </div>
    <?php } ?>
  <?php } ?>
</section>
