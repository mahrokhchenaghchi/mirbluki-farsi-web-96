...
(function () {
  ...
  /* 🔴 پیش‌فرض خاموش (تصمیم مالک)؛ با کلید «صدای آب» روی همین کارت روشن می‌شود */
  var soundOn = false;
  ...
  card.addEventListener('click', function (e) {
    ...
    // پر کردن: صدای آب · خالی‌کردن: بی‌صدا (خواستهٔ مالک)
  ...
  if (finForm) finForm.addEventListener('submit', function (e) {
    ...
    post(null, 'finalize', function () {
      // بدون پرش صفحه: دکمه سرجایش می‌ماند، فقط غیرفعال و متنش عوض می‌شود
...
(function () {
  ...
  function chirp() { if (!soundOn) return; var c = ctx(); if (!c) return; var t = c.currentTime + 0.01;
    tone('sine', 470, 780, t, 0.16, 0.10, 22);
    tone('sine', 620, 900, t + 0.13, 0.18, 0.09, 22);
  }
  ...
  function purr() { if (!soundOn) return; var c = ctx(); if (!c) return; var t = c.currentTime + 0.01;
    tone('triangle', 112, 126, t, 1.05, 0.10, 26);
    noise(t, 1.0, 900, 0.02, 0.7);
    tone('sine', 700, 880, t + 1.0, 0.12, 0.05, 22);
  }
  ...
  stage.addEventListener('click', function (e) {
    ...
    if (n >= 4) {                                  // ۴ لمس: فقط خرخر، بدون حباب
      purr();
      touchAnim();
      return;
    }
