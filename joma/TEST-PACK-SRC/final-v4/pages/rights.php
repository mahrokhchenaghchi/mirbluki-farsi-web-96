 ...
 * سه حق واقعی و کارکردی: خروجی گرفتن (CSV/JSON/گزارش خوانا) · دیدن اینکه چه چیزی ذخیره شده ·
    ...
    if ($act === 'delete_request') {
        ...
        if ($confirm !== 'پاک کن') {
            ...
            if (function_exists('joma_kv_set')) {
                ...
                $err = 'ثبت درخواست در این نصب ممکن نشد.';
            }
        }
...
if (isset($_GET['export'])) {
    ...
    if ($kind === 'report') {
        // گزارش خوانا: صفحهٔ چاپی (کاربر با Ctrl+P به PDF تبدیل می‌کند) — بدون کتابخانهٔ بیرونی
        ...
        <div class="card">
          ...
          <p class="tiny">تاریخ تهیه: <?php echo e(jalali_format($today)); ?> · این گزارش از ثبت‌های واقعی خودت ساخته شده است.</p>
          <div class="btn-row" style="margin-top:8px">
            <button class="btn sm" type="button" onclick="window.print()">چاپ / ذخیره به PDF</button>
            <a class="btn ghost sm" href="<?php echo e(joma_url('index.php?p=rights')); ?>">بازگشت</a>
          </div>
          <p class="tiny">برای گرفتن PDF فارسی از این صفحه، در پنجرهٔ چاپ گزینهٔ «Save as PDF» مرورگر را انتخاب کن.</p>
        </div>
        <?php
        foreach ($allPlans as $pl) {
            ...
            if ($sr && $sr['overall_success'] !== null) {
                ...
                echo '<p class="lede">موفقیت کلی: اعلام نمی‌شود (چرخهٔ کامل یا داده کافی نبود) — عدد جعلی نمی‌سازیم.</p>';
            }
...
<?php if ($delReq) { ?>
  <div class="card" style="border:1.5px solid var(--coral);background:linear-gradient(150deg,#FFFFFF,var(--coral-soft))">
    ...
    <p class="tiny">پاک‌سازی نهایی خودکار به زمان‌بند سرور (cron) نیاز دارد که در این نصب نیست — <b>نمایشی · منتظر بک‌اند</b>. یعنی فعلاً چیزی خودبه‌خود پاک نمی‌شود؛ اگر تا آن زمان لغو نکنی، پشتیبانی طبق همین درخواست پاک‌سازی را انجام می‌دهد.</p>
...
<div class="grid grid-2">
  ...
  <section class="card">
    ...
    <div class="grid" style="grid-template-columns:1fr;gap:8px;margin-top:8px">
      <a class="btn" href="<?php echo e(joma_url('index.php?p=rights&export=report')); ?>">گزارش خوانا (چاپ / PDF)</a>
      <a class="btn sec" href="<?php echo e(joma_url('index.php?p=rights&export=csv')); ?>">صفحه‌گسترده CSV (برای اکسل)</a>
      <a class="btn ghost" href="<?php echo e(joma_url('index.php?p=rights&export=json')); ?>">فایل پشتیبان کامل JSON</a>
    </div>
    <table class="tbl" style="margin-top:10px">
      <thead><tr><th>قالب</th><th>برای چه کاری</th></tr></thead>
      <tbody>
        <tr><td>گزارش خوانا</td><td>دیدن و نشان‌دادن به متخصص (PDF از پنجرهٔ چاپ مرورگر)</td></tr>
        <tr><td>CSV</td><td>تحلیل در اکسل؛ هر ثبت یک سطر</td></tr>
        <tr><td>JSON</td><td>پشتیبان و انتقال بین دستگاه‌ها — برای کاربر فنی</td></tr>
      </tbody>
    </table>
    <p class="tiny">اگر نمی‌دانی JSON چیست، لازم نیست بدانی؛ همان گزارش خوانا را بگیر.</p>
  </section>
</div>
...
<section class="card" style="border:1.5px solid var(--card-brd)">
  ...
  <p class="tiny" style="margin-top:8px">اجرای خودکار پاک‌سازی پس از ۳۰ روز (کرون) در این نصب نیست — <b>نمایشی · منتظر بک‌اند</b>؛ بقیهٔ مسیر واقعی است.</p>
</section>
<?php joma_footer(); ?>
