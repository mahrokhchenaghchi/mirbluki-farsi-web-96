...
$topActs = array();
if (store_mode() === 'mysql') {
    $raw = joma_query('SELECT activity_code, COUNT(*) AS c FROM joma_performance_events GROUP BY activity_code ORDER BY c DESC LIMIT 8', '', array());
...
<section class="card" style="border:1.5px solid var(--coral)">
  ...
  <p class="tiny" style="margin-top:6px">اجرای خودکار پس از مهلت به کرون نیاز دارد — <b>نمایشی · منتظر بک‌اند</b>. کاربر خودش می‌تواند لغو کند.</p>
</section>
<?php } ?>
