...
<section class="card hd-sec" style="animation-delay:.15s;">
  ...
  <?php if (count($hammasir_p_attention) === 0) { ?>
    <p style="color:var(--ok);">✓ همه‌چیز مرتب است.</p>
  <?php } else { ?>
    <?php foreach ($hammasir_p_attention as $hammasir_p_ai) { ?>
      <?php
      $hammasir_p_ai_color = 'var(--gold)';
      if ($hammasir_p_ai['type'] === 'unread') $hammasir_p_ai_color = 'var(--brand)';
      if ($hammasir_p_ai['type'] === 'mood_decline') $hammasir_p_ai_color = 'var(--rose)';
      ?>
      <div class="hd-item">
...
<section class="hd-sec" style="animation-delay:.25s;">
  ...
  <?php if (count($hammasir_p_cards) === 0) { ?>
    <section class="card" style="text-align:center;padding:34px 20px;">
      ...
      <p style="color:var(--ink-2);">مرجعان شما اینجا فهرست می‌شوند. آن‌ها شما را از منوی «هم‌مسیر» انتخاب می‌کنند و شما درخواست را اینجا می‌پذیرید.</p>
    </section>
  <?php } else { ?>
  <div class="hd-cards">
      ...
      <div class="hd-card">
        <div class="btn-row" style="justify-content:space-between;width:100%;">
          ...
          <?php if ($hammasir_p_c['status'] === 'ACTIVE') { ?>
            ...
            <?php } else { ?>
              <span class="chip" style="color:var(--ok);">● فعال</span>
            <?php } ?>
          <?php } else { ?>
            <span class="chip" style="color:var(--gold);">● در انتظار</span>
          <?php } ?>
        </div>
        <?php if ($hammasir_p_c['status'] === 'ACTIVE') { ?>
          ...
          <?php if (is_array($hammasir_p_c['dots'])) { ?>
            <div style="margin:8px 0 2px;font-size:12px;color:var(--ink-2);">خلق:
