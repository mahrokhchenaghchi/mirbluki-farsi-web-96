...
$ui = array(
    ...
    'stress' => array('label' => 'استرس', 'title' => 'امروز چقدر تحت فشار بودی؟', 'hint' => '۱ = آرام · ۵ = پرتنش. این شاخص جدا از بقیه گزارش می‌شود.', 'scene' => 'coral', 'svg' => 'wave',
        'opts' => array(1 => array('', 'آرام'), 2 => array('', 'کم'), 3 => array('', 'متوسط'), 4 => array('', 'زیاد'), 5 => array('', 'پرتنش'))),
);
...
<form method="post" id="mood-wizard" data-start="<?php echo (int) $startStep; ?>">
  ...
  <div class="wdots" role="tablist" aria-label="مراحل ثبت حال">
    ...
    <button type="button" class="wdot" data-step="6" role="tab"><i class="num">۶</i><span>یک جمله</span></button>
  </div>
  ...
  <?php $i = 0; foreach ($ui as $key => $cfg) { $i++; ?>
    <section class="card wstep scene-<?php echo e($cfg['scene']); ?>" data-step="<?php echo $i; ?>">
      <div class="whead">
        <span class="wvisual"><?php
          if (isset($cfg['svg']) && $cfg['svg'] === 'battery') echo joma_v2_mood_svg('battery', $v[$key]);
          elseif (isset($cfg['svg']) && $cfg['svg'] === 'rings') echo joma_v2_mood_svg('rings', $v[$key]);
          elseif (isset($cfg['svg']) && $cfg['svg'] === 'moon') echo joma_v2_mood_svg('moon', $v[$key]);
          elseif (isset($cfg['svg']) && $cfg['svg'] === 'wave') echo joma_v2_mood_svg('wave', $v[$key]);
          else echo joma_v2_mood_svg('face', $v[$key]);
        ?></span>
        <div>
      ...
      <div class="wopts">
        <?php foreach ($cfg['opts'] as $score => $opt) { ?>
          <label class="wopt<?php echo ($v[$key] === $score) ? ' on' : ''; ?>">
            ...
            <?php if ($opt[0] !== '') echo '<span class="wemoji">' . $opt[0] . '</span>'; ?>
  ...
  <section class="card wstep" data-step="6">
    ...
    <p class="tiny" style="margin-top:6px">یادداشت خصوصی است و در هیچ پاسخ مشاور نمی‌آید.</p>
  </section>
