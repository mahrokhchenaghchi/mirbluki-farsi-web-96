...
function joma_water_state($user_id, $date) {
    ...
    if (!is_array($rec) || empty($rec['status'])) {
        // ثبت قدیمی بدون وضعیت (تصمیم مالک):
        //   · امروز → پیش‌نویس قابل ویرایش و قابل «ثبت نهایی»
        //   · گذشته → قطعی (گذشته بازنویسی نمی‌شود)
        $today0 = function_exists('jalali_today') ? jalali_today() : '';
        if ($today0 !== '' && strcmp($date, $today0) === 0) {
            $out['status'] = 'draft';
            $out['effective_final'] = false;
            return $out;
        }
        $out['status'] = 'none';
        $out['effective_final'] = true;
