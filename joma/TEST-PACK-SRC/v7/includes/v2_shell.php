...
function joma_v2_greeting($user) {
    ...
    // بازه‌های سند (SPEC/11 §۰): ۵–۱۰:۵۹ صبح · ۱۱–۱۳:۵۹ ظهر · ۱۴–۱۸:۵۹ عصر · ۱۹–۴:۵۹ شب
    $h = (int) date('G');
    if ($h >= 5 && $h < 11) $part = 'صبح بخیر';
    elseif ($h >= 11 && $h < 14) $part = 'ظهر بخیر';
    elseif ($h >= 14 && $h < 19) $part = 'عصر بخیر';
    else $part = 'شب بخیر';
...
function joma_v2_time_icon_id() {
    // سازگاری: آیکون خطی قدیمی — دیگر استفاده نمی‌شود؛ استیکر جای آن آمد.
    return (joma_v2_time_slot() === 'moon') ? 'i-moon' : 'i-sun';
}
