...
?>
<?php $__owlLib = '<span class="owl-chip">' . joma_v2_icon('owl-hi') . '</span>';
joma_v2_pagehead('کتابخانهٔ جوما', 'کتابخانهٔ فعالیت‌ها', 'فعالیت‌های آمادهٔ جوما؛ هر کدام هدف و واحد خودش را دارد. آن‌چه با زندگی‌ات جور است را به برنامهٔ ماه اضافه کن.', $__owlLib); ?>

<div class="rfilters">
  <a class="fsel<?php echo ($cat === '' && $fr === '' && $q === '') ? ' on' : ''; ?>" href="<?php echo e(joma_url('index.php?p=library')); ?>">همه</a>
  <?php foreach (categories_list() as $c) {
      $on = ($cat === $c) ? ' on' : '';
      echo '<a class="fsel' . $on . '" href="' . e(joma_url('index.php?p=library&cat=' . urlencode($c))) . '">' . e($c) . '</a>';
  } ?>
</div>
...
<form class="card" method="get" style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end">
  ...
  <div style="flex:1;min-width:180px"><label>جست‌وجو</label><input name="q" placeholder="نام یا کد ACT…" value="<?php echo e($q); ?>"></div>
  <div style="flex:1;min-width:160px"><label>تناوب</label><select name="fr"><option value="">همه</option><?php foreach (frequencies_list() as $k => $v) echo '<option value="'.$k.'" '.($fr === $k ? 'selected' : '').'>'.$v.'</option>'; ?></select></div>
  <button class="btn sec">فیلتر</button>
  <a class="btn" href="<?php echo e(joma_url('index.php?p=library&new=1')); ?>"><?php echo joma_v2_icon('i-plus'); ?>فعالیت شخصی</a>
</form>
...
foreach ($list as $a) {
    ...
    echo '<div class="act-head" style="background:'.$a['color'].'33"><div class="sticker">'.e($a['sticker']).'</div>';
    echo '<span class="chip g">'.e(frequencies_list()[$a['frequency']]).'</span>';
    echo '<span style="flex:1"></span><span class="chip" dir="ltr">'.e($a['code']).'</span></div>';
    echo '<div class="act-body"><h3>'.e($a['name']).'</h3>';
    echo '<p class="meta">'.e($a['category']).' · اهمیت '.e(weight_label($a['weight'])).'</p>';
