...
/* ---------- کمک‌تابع نمودار (گام نهایی): میلهٔ افقی با برچسب عددی ---------- */
function joma_reports_bar_chart($canvasId, $title, $labels, $values, $sentence, $unit = '') {
    if (!function_exists('analytics_chartjs_tag')) return;
    echo analytics_chartjs_tag();
    echo '<section class="card">';
    echo '<div class="k">' . e($title) . '</div>';
    if (!$labels || count($labels) === 0) {
        echo '<div class="chart-empty" style="margin-top:8px">داده‌ای برای این نمودار نیست.<br><span class="tiny">وقتی ثبت‌ها شروع شود، این نمودار پر می‌شود.</span></div>';
    } else {
        echo '<div class="chart-box" style="height:' . max(200, 46 * count($labels) + 70) . 'px;margin-top:8px"><canvas id="' . e($canvasId) . '"></canvas></div>';
        echo '<p class="tiny" style="margin-top:6px">' . e($sentence) . '</p>';
        $spec = array('labels' => array_values($labels), 'values' => array_map('floatval', array_values($values)), 'unit' => $unit);
        echo '<script>(function(){var spec=' . json_encode($spec, JSON_UNESCAPED_UNICODE) . ';'
           . 'function draw(){if(typeof Chart==="undefined"){return;}var el=document.getElementById(' . json_encode($canvasId) . ');if(!el)return;'
           . 'if(Chart.getChart&&Chart.getChart(el)){Chart.getChart(el).destroy();}'
           . 'new Chart(el,{type:"bar",data:{labels:spec.labels,datasets:[{label:spec.unit||"مقدار",data:spec.values,backgroundColor:"#0FA678",borderRadius:6}]},'
           . 'options:{indexAxis:"y",responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},'
           . 'tooltip:{rtl:true,callbacks:{label:function(c){return c.parsed.x+(spec.unit?(" "+spec.unit):"");}}}},' 
           . 'scales:{x:{beginAtZero:true,ticks:{color:"#5C7A6C",font:{family:"Vazirmatn,Tahoma,sans-serif",size:10.5}},grid:{color:"rgba(20,56,46,.10)"}},'
           . 'y:{ticks:{color:"#14382E",font:{family:"Vazirmatn,Tahoma,sans-serif",size:11}},grid:{display:false}}}}});}'
           . 'if(document.readyState!=="loading"){setTimeout(draw,60);}else{document.addEventListener("DOMContentLoaded",function(){setTimeout(draw,60);});}})();</script>';
    }
    echo '</section>';
}


/** نمودار دوسری (مقایسهٔ دو دوره) — فقط اعداد واقعی دو طرف. */
function joma_reports_grouped_chart($canvasId, $title, $labels, $seriesA, $seriesB, $labelA, $labelB, $sentence) {
    if (!function_exists('analytics_chartjs_tag')) return;
    echo analytics_chartjs_tag();
    echo '<section class="card">';
    echo '<div class="k">' . e($title) . '</div>';
    if (!$labels) {
        echo '<div class="chart-empty" style="margin-top:8px">برای مقایسه، دادهٔ هر دو دوره لازم است.</div>';
    } else {
        echo '<div class="chart-box" style="height:260px;margin-top:8px"><canvas id="' . e($canvasId) . '"></canvas></div>';
        echo '<p class="tiny" style="margin-top:6px">' . e($sentence) . '</p>';
        $spec = array('labels' => array_values($labels), 'a' => array_values($seriesA), 'b' => array_values($seriesB), 'la' => $labelA, 'lb' => $labelB);
        echo '<script>(function(){var s=' . json_encode($spec, JSON_UNESCAPED_UNICODE) . ';'
           . 'function draw(){if(typeof Chart==="undefined"){return;}var el=document.getElementById(' . json_encode($canvasId) . ');if(!el)return;'
           . 'if(Chart.getChart&&Chart.getChart(el)){Chart.getChart(el).destroy();}'
           . 'new Chart(el,{type:"bar",data:{labels:s.labels,datasets:[{label:s.la,data:s.a,backgroundColor:"#0FA678",borderRadius:6},{label:s.lb,data:s.b,backgroundColor:"#8B7CF6",borderRadius:6}]},'
           . 'options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:"#5C7A6C",font:{family:"Vazirmatn,Tahoma,sans-serif",size:11}}},tooltip:{rtl:true}},'
           . 'scales:{y:{beginAtZero:true,ticks:{color:"#5C7A6C",font:{family:"Vazirmatn,Tahoma,sans-serif",size:10.5}},grid:{color:"rgba(20,56,46,.10)"}},'
           . 'x:{ticks:{color:"#14382E",font:{family:"Vazirmatn,Tahoma,sans-serif",size:10.5}},grid:{display:false}}}}});}'
           . 'if(document.readyState!=="loading"){setTimeout(draw,60);}else{document.addEventListener("DOMContentLoaded",function(){setTimeout(draw,60);});}})();</script>';
    }
    echo '</section>';
}

/* جمع‌بندی تب «خلاصه» برای نمودار روند */
joma_header('گزارش‌ها', array());
?>
<?php joma_v2_pagehead('گزارش‌ها', 'گزارش‌های من', 'فقط دادهٔ دورهٔ ' . jalali_period_label($key) . '.', '<span>' . e(jalali_period_label($key)) . '</span>'); ?>
...
if (in_array($tab, $needsData, true) && !$hasEvents && !$rep['moods'] && !$rep['activities']) {
    echo '<div class="nodata"><span class="ico">🌱</span><b>برای این دوره داده‌ای ثبت نشده است</b>'
       . '<p class="lede">وقتی چند روز ثبت کنی، این تب پر می‌شود.</p>'
...
if ($tab === 'overview') {
    ...
    // نمودار: روند روزانهٔ موفقیت (همان سری تب «روند»، اینجا هم دیده شود)
    if ($sr !== null) {
        render_analytics_daily_chart($sr, 'ch-overview');
    }
    ...
    if (!$rep['activities']) {
        ...
        // نمودار: درصد تحقق هر فعالیت (مقدار واقعی ÷ هدف)
        $chLabels = array(); $chValues = array();
        foreach ($rep['activities'] as $a) {
            $pct = ((float) $a['target_value'] > 0) ? min(100, round(((float) $a['actual'] / (float) $a['target_value']) * 100)) : 0;
            $chLabels[] = $a['sticker'] . ' ' . $a['name'];
            $chValues[] = $pct;
        }
        joma_reports_bar_chart('ch-acts', 'درصد تحقق هر فعالیت',
            $chLabels, $chValues,
            'هر میله یعنی چند درصد از هدفِ همان فعالیت را محقق کرده‌ای. روزهای بدون ثبت صفر حساب نمی‌شوند؛ فقط مقدارهای ثبت‌شده در این عدد می‌آیند.',
            '٪');
    }
} elseif ($tab === 'weight') {
    ...
    if (!$rep['activities']) {
        ...
        $wLabels = array(); $wValues = array();
        foreach ($rep['activities'] as $a) { $wLabels[] = $a['sticker'] . ' ' . $a['name']; $wValues[] = (int) $a['weight']; }
        joma_reports_bar_chart('ch-weight', 'سهم هر فعالیت در موفقیت کلی (وزن)',
            $wLabels, $wValues,
            'هر چه میله بلندتر باشد، آن فعالیت سهم بیشتری از «موفقیت کلی» دارد. وزن را خودت در برنامهٔ همان ماه تعیین کرده‌ای.',
            'وزن');
    }
} elseif ($tab === 'cal') {
    $calS = success_report($rep['plan'], $rep['bounds'], $rep['activities'], $rep['events'], jalali_today());
    render_analytics_heatmap($calS, $rep['moods']);
    $wkLabels = array(); $wkValues = array();
    foreach ($rep['calendar'] as $c) {
        $d = (int) substr($c['date'], 8, 2);
        $w = (int) ceil($d / 7);
        if (!isset($wkValues[$w])) { $wkValues[$w] = 0; $wkLabels[$w] = 'هفتهٔ ' . $w; }
        $wkValues[$w] += (int) $c['event_count'];
    }
    ksort($wkValues);
    joma_reports_bar_chart('ch-cal-weeks', 'رخدادهای هر هفتهٔ ماه',
        array_values($wkLabels), array_values($wkValues),
        'جمع رخدادهای ثبت‌شده در هر هفته. نقشهٔ بالا شدت هر روز را نشان می‌دهد و این میله‌ها جمع هر هفته را.', 'رخداد');
} elseif ($tab === 'trend') {
    ...
    if (count($list) < 2) {
        ...
        $cl = array(); $ca = array(); $cb = array();
        foreach (array(array('موفقیت کلی (٪)', 'success', 100), array('پوشش داده (٪)', 'coverage', 100),
                       array('میانگین حال (۱..۵)', 'mood', 1), array('میانگین استرس (۱..۵)', 'stress', 1)) as $pr) {
            $va = isset($sumA[$pr[1]]) ? $sumA[$pr[1]] : null;
            $vb = isset($sumB[$pr[1]]) ? $sumB[$pr[1]] : null;
            if ($va === null && $vb === null) continue;
            $cl[] = $pr[0];
            $ca[] = ($va === null) ? 0 : round((float) $va * $pr[2], 1);
            $cb[] = ($vb === null) ? 0 : round((float) $vb * $pr[2], 1);
        }
        joma_reports_grouped_chart('ch-cmp', 'مقایسهٔ دو دوره در یک نگاه',
            $cl, $ca, $cb, jalali_period_label($key), jalali_period_label($other),
            'در هر جفت، یک میله این دوره است و یکی دورهٔ دیگر. اگر شاخصی در دوره‌ای محاسبه نشده باشد صفر نشان داده می‌شود — یعنی «داده کافی نبوده»، نه اینکه صفر بوده.');
    ...
    if ($__analytics_r1_ok) {
        if ($sr !== null) {
            if ($rep['moods']) {
                render_analytics_mood($sr, $rep['moods'], array('canvas' => 'ch-an-mood', 'toggle' => 'mood-toggle-an'));
            } else {
                echo '<section class="card"><div class="k">خلق در طول دوره</div>'
                   . '<div class="chart-empty" style="margin-top:8px">هنوز حالی ثبت نشده است؛ با ثبت روزانه این نمودار پر می‌شود.</div></section>';
            }
        }
        try {
    ...
    if (!$rep['activities']) {
        ...
        $dLabels = array(); $dValues = array();
        foreach ($rep['activities'] as $a) { if ((int) $a['event_count'] === 0) continue; $dLabels[] = $a['sticker'] . ' ' . $a['name']; $dValues[] = (int) $a['event_count']; }
        joma_reports_bar_chart('ch-det', 'تعداد رخدادهای ثبت‌شده هر فعالیت',
            $dLabels, $dValues,
            'این نمودار فقط «چند بار ثبت کرده‌ای» را می‌شمارد، نه کیفیت کارها. رخداد خام، ساده‌ترین شکل دادهٔ توست.',
            'رخداد');
        foreach ($rep['activities'] as $a) {
