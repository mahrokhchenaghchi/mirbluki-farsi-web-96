...
?>
<?php joma_v2_pagehead('جوجهٔ من', $jooje_title, 'هرچه این‌جا می‌بینی، از ثبت‌های خودت ساخته می‌شود.', '<span>' . e($stage) . ' · ' . e($state) . '</span>'); ?>

<section class="joma-msg">
  <span class="av owl-float" data-owl-tap><?php echo joma_v2_icon('owl-hi', 'ic lg'); ?></span>
  <span style="flex:1;min-width:0"><b>جوما</b><p class="lede">این‌جا اتاق شخصی توست؛ من فقط نگه‌بانِ همین صفحه‌ام — جوجه با ثبت‌های خودت بزرگ می‌شود.</p></span>
</section>
...
<div class="grid grid-2">
  <section class="card" style="padding:14px">
    ...
    <div class="pet-btns">
      <button type="button" class="btn sm" data-pet-pet>نوازش</button>
      ...
      <div style="margin-top:10px">
        ...
        <?php } else { ?>
          <?php if (!empty($m['target']) && $m['progress'] !== null) { ?>
            ...
            ?></p>
          <?php } else { ?>
            <p class="tiny">برای این سنجه هدف روزانه تعیین نشده؛ فقط عدد واقعی خودت نشان داده می‌شود.</p>
          <?php } ?>
        <?php } ?>
      </div>
    <?php } ?>
