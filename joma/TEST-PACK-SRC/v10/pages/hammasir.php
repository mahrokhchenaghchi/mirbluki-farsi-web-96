...
// (v10) موتور «کد دعوت یک‌بارمصرف» — فایل تازه؛ اگر نبود، صفحه‌های ماژول مثل قبل کار می‌کنند.
$_joma_invcode = dirname(__FILE__) . '/../functions/companion_invitecode.php';
if (is_file($_joma_invcode)) require_once $_joma_invcode;
unset($_joma_invcode);
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (is_array($hammasir_my_links)) {
                foreach ($hammasir_my_links as $hammasir_l) {
                    if ((int) $hammasir_l['id'] === $hammasir_link_id) {
                        ...
                        // (C3) دکمهٔ «پیام دادن» کارت مراجع: گفت‌وگو همان لحظه باز شود
                        if (!empty($_POST['open_chat'])) $_SESSION['hammasir_prov_chat'] = 1;
                        else unset($_SESSION['hammasir_prov_chat']);
    ...
    // --- (بند ۴) کارت دعوت «انتخاب هم‌مسیر» در خانهٔ من ---
    //     «بعداً» → برای همیشه می‌رود؛ «انتخاب هم‌مسیر» → همان لحظه دیده‌شده ثبت می‌شود.
    if ($hammasir_action === 'invite_later') {
        if (function_exists('joma_invite_mark_done')) joma_invite_mark_done($hammasir_me_post);
        try { hammasir_user_flag_set($hammasir_me_post, 'onboarding_intent', 0); } catch (Throwable $e) { /* بی‌اثر */ }
        joma_redirect('index.php?p=dashboard');
    }
    if ($hammasir_action === 'invite_open') {
        if (function_exists('joma_invite_open_intent')) joma_invite_open_intent($hammasir_me_post);
        joma_redirect('index.php?p=hammasir');
    }
    // --- (v10) کد دعوت یک‌بارمصرف — ساخت (مشاور) / لغو (مشاور) / پذیرش (کاربر) ---
    if ($hammasir_action === 'invite_code_create') {
        if (!function_exists('joma_invitecode_create')) {
            flash_set('err', 'امکان انجام این عملیات در حال حاضر وجود ندارد.');
            joma_redirect('index.php?p=hammasir');
        }
        $hammasir_code = 'error';
        try {
            $hammasir_code = joma_invitecode_create($hammasir_me_post, $hammasir_me_post);
        } catch (Throwable $e) {
            $hammasir_code = 'error';
        }
        if ($hammasir_code !== 'error' && $hammasir_code !== 'too_many' && $hammasir_code !== 'invalid_provider') {
            flash_set('ok', 'کد ساخته شد: ' . $hammasir_code . ' — این کد ۷ روز اعتبار دارد، یک‌بار مصرف می‌شود، و تا وقتی خودِ کاربر آن را وارد نکند هیچ داده‌ای دیده نمی‌شود.');
        } else {
            $hammasir_msgs_code = function_exists('joma_invitecode_messages') ? joma_invitecode_messages() : array();
            flash_set('err', isset($hammasir_msgs_code[$hammasir_code]) ? $hammasir_msgs_code[$hammasir_code] : 'امکان انجام این عملیات در حال حاضر وجود ندارد.');
        }
        joma_redirect('index.php?p=hammasir');
    }
    if ($hammasir_action === 'invite_code_revoke') {
        $hammasir_code_id = isset($_POST['code_id']) ? (int) $_POST['code_id'] : 0;
        $hammasir_res = function_exists('joma_invitecode_revoke') ? joma_invitecode_revoke($hammasir_me_post, $hammasir_code_id) : 'error';
        flash_set(($hammasir_res === 'ok') ? 'ok' : 'err', ($hammasir_res === 'ok') ? 'کد لغو شد.' : 'امکان انجام این عملیات در حال حاضر وجود ندارد.');
        joma_redirect('index.php?p=hammasir');
    }
    if ($hammasir_action === 'invite_code_redeem') {
        $hammasir_pv_code = array(
        ...
        $hammasir_code_res = 'error';
        try {
            $hammasir_code_res = joma_invitecode_redeem(
                $hammasir_me_post,
                isset($_POST['invite_code']) ? $_POST['invite_code'] : '',
                $hammasir_pv_code,
                isset($_POST['perm_msg_out']),
                isset($_POST['perm_msg_in']),
                (isset($_POST['status_share']) && (string) $_POST['status_share'] === '1')
            );
        } catch (Throwable $e) {
            $hammasir_code_res = 'error';
        }
        if ($hammasir_code_res === 'ok' && function_exists('joma_invite_mark_done')) {
            joma_invite_mark_done($hammasir_me_post); // کارت دعوت دیگر نمی‌آید
        }
        $hammasir_msgs_code = function_exists('joma_invitecode_messages') ? joma_invitecode_messages() : array();
        $hammasir_known = isset($hammasir_msgs_code[$hammasir_code_res]);
        if ($hammasir_code_res !== 'ok' && $hammasir_known) {
            flash_set('err', $hammasir_msgs_code[$hammasir_code_res]);
        } elseif ($hammasir_code_res !== 'ok') {
            flash_set('err', 'امکان انجام این عملیات در حال حاضر وجود ندارد.');
        } else {
            flash_set('ok', $hammasir_msgs_code['ok']);
        }
        joma_redirect('index.php?p=hammasir');
    }

    // --- (C3) نشانگر وضعیت ثبت: کادر مستقل، یک‌کلیک خاموش/روشن ---
    if ($hammasir_action === 'status_share_set') {
        $hammasir_share_on = (isset($_POST['enabled']) && (string) $_POST['enabled'] === '1');
        if (function_exists('joma_status_share_set')) {
            joma_status_share_set($hammasir_me_post, $hammasir_share_on);
            flash_set('ok', $hammasir_share_on
                ? 'از این به بعد همراهت می‌داند چند وقت است چیزی ثبت نکرده‌ای.'
                : 'از این به بعد همراهت نشانگر وضعیت ثبت را نمی‌بیند.');
        }
        joma_redirect('index.php?p=hammasir');
    }
    ...
    if ($hammasir_action === 'mode_set') {
        ...
        if ($hammasir_result === 'ok') {
            ...
            $hammasir_dest = ($hammasir_mode_req === 'client') ? 'index.php?p=dashboard' : 'index.php?p=hammasir';
            // (C2) پنل «نقش‌های من» می‌تواند مقصد را تعیین کند — فقط صفحه‌های مجاز
            $hammasir_to = isset($_POST['redirect_to']) ? (string) $_POST['redirect_to'] : '';
            $hammasir_allowed_to = array('dashboard', 'hammasir', 'admin_console', 'today', 'reports', 'journal', 'library', 'learn', 'settings', 'profile', 'rights', 'periods', 'mood', 'jooje', 'plan');
            if ($hammasir_to !== '' && in_array($hammasir_to, $hammasir_allowed_to, true)) {
                // مقصد «صفحه‌های شخصی» فقط در حالت کاربری معنا دارد (C2 §۲٫۲٫۱)
                if (!in_array($hammasir_to, array('mood', 'jooje', 'plan'), true) || $hammasir_mode_req === 'client') {
                    $hammasir_dest = 'index.php?p=' . $hammasir_to;
                }
            }
            if ($hammasir_mode_req === 'admin' && $hammasir_to === '') $hammasir_dest = 'index.php?p=admin_console';
            joma_redirect($hammasir_dest);
        }
        flash_set('err', 'امکان انجام این عملیات در حال حاضر وجود ندارد.');
        joma_redirect('index.php?p=hammasir');
    }
    ...
    if ($hammasir_action === 'provider_register' || $hammasir_action === 'provider_set_title' || $hammasir_action === 'provider_set_status' || $hammasir_action === 'admin_promote' || $hammasir_action === 'admin_demote') {
        ...
        try {
            if ($hammasir_action === 'link_request') {
                ...
                // (بند ۴) گام ۲ دعوت: مجوزهای پیام + کادر مستقل نشانگر وضعیت.
                // خودِ درخواست را همان تابع رسمی ماژول می‌سازد؛ این‌ها بعدش نوشته می‌شوند.
                $hammasir_msg_out = isset($_POST['perm_msg_out']);
                $hammasir_msg_in = isset($_POST['perm_msg_in']);
                $hammasir_share = (isset($_POST['status_share']) && (string) $_POST['status_share'] === '1');
                if (function_exists('joma_companion_link_request')) {
                    $hammasir_result = joma_companion_link_request(
                        $hammasir_me_post,
                        isset($_POST['provider_user_id']) ? (int) $_POST['provider_user_id'] : 0,
                        $hammasir_pv,
                        $hammasir_msg_out,
                        $hammasir_msg_in,
                        $hammasir_share
                ...
                if ($hammasir_result === 'ok' && function_exists('joma_invite_mark_done')) {
                    joma_invite_mark_done($hammasir_me_post); // کارت دعوت دیگر نمی‌آید
                }
                if ($hammasir_result === 'ok') {
...
?>
<?php
// سرصفحهٔ طرح تازه + نشانگرهای واقعی (پیام/رویداد/درخواست) — همان داده‌ها، بدون تغییر منطق
// (C3) سرصفحهٔ هر حالت، متن خودش را دارد: مشاور «مراجعان من»، مدیر «کنسول هم‌مسیر»، مراجع «هم‌مسیر»
$hammasir_head_title = 'هم‌مسیر';
$hammasir_head_sub = 'مسیر همراهی: مراجع می‌بیند چه کسی کنارش است، مشاور فقط آنچه اجازه داده شده، و مدیر همه‌چیز را می‌بیند.';
if ($hammasir_mode === 'provider') {
    $hammasir_head_title = 'مراجعان من';
    $hammasir_head_sub = 'هر مراجع، فقط بخش‌هایی که خودش اجازه داده است.';
} elseif ($hammasir_mode === 'admin') {
    $hammasir_head_title = 'کنسول هم‌مسیر';
}
joma_v2_pagehead(
    'هم‌مسیر',
    $hammasir_head_title,
    $hammasir_head_sub,
    '<span>' . e($hammasir_mode_labels[$hammasir_mode]) . '</span>'
);
?>
<section class="card hero-card">
  <?php if ($hammasir_today !== '') { ?><p class="tiny"><?php echo e($hammasir_today); ?></p><?php } ?>
  <div class="row" style="justify-content:space-between">
    <b style="font-size:13.5px"><?php echo e($hammasir_u['full_name'] ? $hammasir_u['full_name'] : $hammasir_u['username']); ?></b>
    <span class="chip g">حالت: <?php echo e($hammasir_mode_labels[$hammasir_mode]); ?></span>
  </div>
