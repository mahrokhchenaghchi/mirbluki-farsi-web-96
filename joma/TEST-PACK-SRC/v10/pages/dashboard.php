...
$doneToday = 0;
$dailyTotal = 0; // (F1) مخرج حلقهٔ پیشرفت — فقط فعالیت‌های روزانه
foreach ($acts as $a) {
    $rel = events_for($evs, $a['id']);
    if ($a['frequency'] !== 'DAILY') continue;
    $dailyTotal++;
    if (has_daily_registration($rel, $today)) $doneToday++;
}
// (F1) حلقه فقط با عدد واقعی؛ اگر فعالیت روزانه‌ای نیست، عددی وجود ندارد ⇒ حلقه رسم نمی‌شود
$ringPct = ($dailyTotal > 0) ? (int) round($doneToday * 100 / $dailyTotal) : null;
// (F1) حلقهٔ پیشرفت — فایل تازهٔ همین دور؛ اگر نبود، کارت مثل قبل بدون حلقه می‌ماند
$__rings = dirname(__FILE__) . '/../includes/v2_rings.php';
if (is_file($__rings)) require_once $__rings;
unset($__rings);
...
<section class="card hero-card" style="display:flex;align-items:center;gap:14px">
  <span style="flex:none;display:flex;align-items:center;justify-content:center;width:60px;height:60px;border-radius:18px;background:var(--brand-softer)">
    <?php echo function_exists('joma_v2_time_sticker') ? joma_v2_time_sticker(null, 44) : joma_v2_icon(joma_v2_time_icon_id(), 'ic lg'); ?>
  </span>
...
<?php if ($owlMsg !== '') { ?>
<section class="joma-msg" id="joma-msg" role="button" tabindex="0" aria-label="جوما — راهنما">
  <span class="av" data-owl-tap><?php echo joma_v2_icon('owl-hi', 'ic lg'); ?></span>
  <span style="flex:1;min-width:0"><b>جوما</b><p class="lede"><?php echo e($owlMsg); ?></p></span>
</section>
<?php } ?>
...
<div class="grid grid-2">
  ...
  <section class="card" style="border:1.5px solid var(--brand2);background:linear-gradient(150deg,#FFFFFF,var(--brand-softer))">
    <div class="pcard-row">
      ...
      <?php /* (F1) حلقهٔ پیشرفت — فقط وقتی بک‌اند عدد داده باشد (SPEC/03 §۳) */
      if (function_exists('joma_v2_mring')) echo joma_v2_mring($ringPct, 'از کارهای امروز'); ?>
    </div>
  </section>
        ...
        if (!empty($HAMMASIR_CONFIG['hammasir_enabled'])) {
            ...
            if (is_file($__hfn)) {
                ...
                if ($__hmode === 'admin') {
                    ...
                    // (بند ۴) کارت دعوت «انتخاب هم‌مسیر» — کارت درون‌صفحه، یک‌بار.
                    // منبع تصمیم: پرچم بک‌اند (joma_invite_state) — فرانت حدس نمی‌زند.
                    // نه مودال، نه پاپ‌آپ، نه نوار چسبان، نه بَج، نه یادآور.
                    $__hinv = 'HIDDEN';
                    try {
                        if (function_exists('joma_invite_state')) $__hinv = joma_invite_state((int) $u['id']);
                    } catch (Throwable $e) {
                        $__hinv = 'HIDDEN'; // fail-closed: کارت رندر نمی‌شود
                    }
                    if ($__hinv === 'READY') {
                        echo '<section class="card inv-card" id="comp-invite">';
                        echo '<div class="inv-art"><span class="floaty">' . joma_v2_icon('owl-hi', 'ic lg') . '</span>'
                           . '<i class="inv-dot d1"></i><i class="inv-dot d2"></i><i class="inv-dot d3"></i></div>';
                        echo '<div class="inv-body">';
                        echo '<span class="inv-tag">' . joma_v2_icon('i-users') . 'یک پیشنهاد، فقط همین یک‌بار</span>';
                        echo '<h3>یک نفر را کنارت بیاور؟</h3>';
                        echo '<p>هم‌مسیر می‌تواند خلاصهٔ مسیرت را ببیند و کنارت بماند — و <b>فقط چیزهایی که خودت روشن کنی</b>. '
                           . 'تا خودش قبول نکند و تو تأیید نکنی، هیچ‌چیز رد و بدل نمی‌شود.</p>';
                        echo '<div class="inv-acts">';
                        echo '<form method="post" action="' . e(joma_url('index.php?p=hammasir')) . '">' . csrf_field()
                           . '<input type="hidden" name="hammasir_action" value="invite_open">'
                           . '<button class="btn" type="submit">انتخاب هم‌مسیر</button></form>';
                        echo '<form method="post" action="' . e(joma_url('index.php?p=hammasir')) . '">' . csrf_field()
                           . '<input type="hidden" name="hammasir_action" value="invite_later">'
                           . '<button class="btn ghost" type="submit">بعداً</button></form>';
                        echo '</div>';
                        echo '<div class="inv-note">این کارت دیگر خودش نمی‌آید. هر وقت خواستی، از منو ← «هم‌مسیر».</div>';
                        echo '</div></section>';
                    }
                }
            }
...
<?php
/* «مسیر این ماه» — تقویم سه‌بعدی پاستلی (SPEC/11 §۷)؛ همان کامپوننتی که در کارهای امروز هم می‌آید. */
if (function_exists('joma_v2_month_calendar')) {
    joma_v2_month_calendar((int) $u['id'], $plan, $acts, $evs, $moods, array('title' => true, 'counters' => true, 'stats' => true));
}
?>
