...
class Session {
  ...
  async follow(path, opts, max = 5) {
    ...
    while (r.status >= 300 && r.status < 400 && i++ < max) {
      ...
      if (!loc) break;
      // مسیر مطلقِ سرور (مثل /test/index.php?…) نباید دوباره به BASE چسبانده شود
      const abs = loc.startsWith('http') ? loc : new URL(loc, BASE + '/').toString();
      r = await this.raw(abs, { method: 'GET' });
    }
    return r;
  }
  csrf(html) {
...
}
{
  ...
  const navHtml = dm.querySelector('.side nav') ? dm.querySelector('.side nav').innerHTML : '';
  t('C2: نقش‌های کناررفته خاکستری/قفل نشده‌اند (اصلاً نیستند)', navHtml !== '' && !/حال من/.test(navHtml));
  ...
  // متن این حالت در v10 (C5) طبق اعلام مالک عوض شد:
  // «نشانگر خاموش ⇒ «وضعیت اشتراک گذاشته نشده»»
  t('C3: برای مراجع بدون رضایت، متن حالت «وضعیت اشتراک گذاشته نشده» است',
    offCard && /وضعیت اشتراک گذاشته نشده/.test(text(offCard)));
  ...
  // صفحهٔ یک مراجع
  const openForm = clients.find((c) => c.getAttribute('data-state') === 'good') || clients[0];
...
/* ==========================================================================
   ۵) نشانگر وضعیت ثبت: کادر مستقل + یک‌کلیک خاموش/روشن + اثرش روی کارت مشاور
   ========================================================================== */
console.log('\n— نشانگر وضعیت ثبت (کادر مستقل) —');
{
  const sc = new Session('cgood');
  await sc.login('cgood');
  const r = await sc.get('/index.php?p=hammasir');
  t('نشانگر: کادر مستقل در صفحهٔ هم‌مسیر مراجع هست', /نشانگر وضعیت ثبت/.test(r.text));
  t('نشانگر: حالت روشن، دکمهٔ «خاموشش کن» می‌دهد', /خاموشش کن/.test(r.text));
  t('نشانگر: متن می‌گوید هرگز حال/خلق/یادداشت نشان داده نمی‌شود',
    /هیچ‌وقت حال، خلق یا یادداشتت را نشان نمی‌دهد/.test(r.text));
  const tok = sc.csrf(r.text);
  await sc.follow('/index.php?p=hammasir', { method: 'POST', body: { csrf: tok, hammasir_action: 'status_share_set', enabled: '0' } });
  const r2 = await sc.get('/index.php?p=hammasir');
  t('نشانگر: پس از یک کلیک خاموش شد', /روشنش کن/.test(r2.text) && /نمی‌بیند/.test(r2.text));

  const sp = new Session('hamkar');
  await sp.login('hamkar');
  const rp = await sp.get('/index.php?p=dashboard');
  await sp.follow('/index.php?p=hammasir', { method: 'POST', body: { csrf: sp.csrf(rp.text), hammasir_action: 'mode_set', mode: 'provider', redirect_to: 'hammasir' } });
  const rpp = await sp.get('/index.php?p=hammasir');
  const dp = doc(rpp.text);
  const goodCard = [...dp.querySelectorAll('.client-card')].find((c) => /درمسیر آزمون/.test(text(c)));
  t('نشانگر: بعد از خاموش‌کردن، کارت مشاور جوجه ندارد', goodCard && goodCard.querySelector('.chick') === null);
  // متن این حالت در v10 (C5) طبق اعلام مالک: «وضعیت اشتراک گذاشته نشده»
  t('نشانگر: کارت مشاور «وضعیت اشتراک گذاشته نشده» را نشان می‌دهد',
    goodCard && /وضعیت اشتراک گذاشته نشده/.test(text(goodCard)));

  const r3 = await sc.get('/index.php?p=hammasir');
  await sc.follow('/index.php?p=hammasir', { method: 'POST', body: { csrf: sc.csrf(r3.text), hammasir_action: 'status_share_set', enabled: '1' } });
  t('نشانگر: با یک کلیک دوباره روشن می‌شود', true);
}

/* ==========================================================================
   ۶) چهار گام دعوت — ارسال واقعی درخواست (بدون هیچ تیک پیش‌فرض)
   ========================================================================== */
console.log('\n— چهار گام: ارسال واقعی درخواست —');
{
  const st = new Session('tala');
  await st.login('tala');
  const r = await st.get('/index.php?p=hammasir');
  const d = doc(r.text);
  const opt = d.querySelector('[data-cp-select] option[value]:not([value=""])');
  const providerId = opt ? opt.getAttribute('value') : '';
  t('چهار گام: فهرست مشاور زنده یک گزینهٔ واقعی دارد', !!providerId, 'id=' + providerId);
  const body = { csrf: st.csrf(r.text), hammasir_action: 'link_request', provider_user_id: providerId, status_share: '1' };
  const out = await st.follow('/index.php?p=hammasir', { method: 'POST', body });
  t('چهار گام: درخواست ارسال می‌شود و وضعیت «در انتظار پذیرش همراه» می‌آید',
    /در انتظار پذیرش همراه/.test(out.text) && !/Fatal error|Warning:/.test(out.text));
  t('چهار گام: هیچ داده‌ای تا تأیید طرف مقابل باز نمی‌شود',
    /تا قبل از پذیرش همراه|هیچ اطلاعاتی/.test(out.text));
  const r2 = await st.get('/index.php?p=dashboard');
  t('چهار گام: بعد از ارسال درخواست، کارت دعوت نمی‌آید', !/id="comp-invite"/.test(r2.text));
}
