...
function session_user_array($u) {
    return array(
        ...
        // (B1 — مرحلهٔ ۲): شمارندهٔ نشست؛ با هر تغییر رمز بالا می‌رود و نشست‌های قدیمی باطل می‌شوند.
        'auth_epoch' => function_exists('joma_auth_epoch') ? joma_auth_epoch((int) $u['id']) : 0,
    );
}
...
function can_register_performance($plan, $pa, $date, $value, $existing) {
    ...
    // (تصمیم مالک — B3): ثبت عملکرد برای روزهای آینده بسته است.
    if (strcmp($date, jalali_today()) > 0) return 'ثبت عملکرد برای روزهای آینده ممکن نیست.';
